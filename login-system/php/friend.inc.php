...
public function request_notification($my_id, $send_data){
    try{
        $sql = "SELECT sender, usersName FROM `friend_request` JOIN users ON friend_request.sender = users.usersId WHERE receiver = ?";
...
public function make_friends($my_id, $user_id){
    ...
    try{
        ...
        $delete_pending_friends = "DELETE FROM `friend_request`
         WHERE (sender = :my_id AND receiver = :frnd_id) OR (sender = :frnd_id AND receiver = :my_id)";
            ...
            if($send_data){
                ...
                foreach($all_users as $row){
                    if($row->user_one == $my_id){
                        $get_user = "SELECT usersId, usersName FROM `users` WHERE usersId = ?";
